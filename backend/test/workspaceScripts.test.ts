...
describe("workspace Node scripts", () => {
  ...
  it("verifies release tags and workflow inputs against the backend package version", () => {
    ...
    expect(
      verifyReleaseVersion({
        packageName: "@misaka09982/code-wiki",
    ...
    ).toBe("Release version verified for @misaka09982/code-wiki@0.7.0.");
  });
  ...
  it("rejects npm release versions that do not match the backend package", () => {
    ...
    ).toThrow("does not match @misaka09982/code-wiki@0.7.0");
    expect(() =>
      verifyReleaseVersion({
        packageName: "@misaka09982/code-wiki",
        packageVersion: "0.7.0",
  ...
  it("keeps the npm publish workflow guarded by backend, frontend, release, and package checks", () => {
    ...
    expect(workflow).toContain("npm --prefix backend run verify");
    expect(workflow).toContain("npm --prefix frontend run lint");
    expect(workflow).toContain("npm --prefix backend run release:verify");
    expect(workflow).toContain("npm --prefix backend run pack:smoke");
    expect(workflow.indexOf("npm --prefix backend run verify")).toBeLessThan(
      workflow.indexOf("npm publish --provenance --access public"),
    );
    expect(
      workflow.indexOf("npm --prefix backend run release:verify"),
    ).toBeLessThan(
      workflow.indexOf("npm publish --provenance --access public"),
    );
  });
  ...
  it("runs npm pack dry-runs through an isolated cache wrapper", () => {
    ...
    const dryRunScript = readFileSync(
      resolve(root, "backend/scripts/package-dry-run.mjs"),
      "utf8",
    );
  ...
  it("keeps TypeScript as the default backend and only publishable package", () => {
    ...
    expect(makefile).toContain("BACKEND_DIR := backend");
    ...
    expect(makefile).toContain("test-scripts:");
    expect(makefile).toContain("$(PYTHON) -m pytest -q");

    expect(testWorkflow).toContain("npm --prefix backend run typecheck");
    expect(testWorkflow).toContain("npm --prefix backend test");
    expect(testWorkflow).toContain("npm --prefix backend run pack:smoke");
