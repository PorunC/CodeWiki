...
async def generate_catalog(repo_id: str) -> dict[str, object]:
    try:
        catalog = await _generator().generate_catalog(repo_id)
...
async def generate_pages(repo_id: str) -> dict[str, object]:
    try:
        results = await _generator().generate_all_pages(repo_id)
    except ValueError as exc:
        raise _http_error(exc) from exc
    return {
        "repo_id": repo_id,
...
async def regenerate_page(repo_id: str, slug: str) -> dict[str, object]:
    try:
        result = await _generator().regenerate_page(repo_id, slug)
    except ValueError as exc:
        raise _http_error(exc) from exc
...
async def get_wiki(repo_id: str) -> dict[str, object]:
    ...
    catalog = store.get_latest_doc_catalog(repo_id)
    pages = store.list_doc_pages(repo_id)
    return {
        "repo_id": repo_id,
...
async def get_page(repo_id: str, slug: str) -> dict[str, object]:
    page = get_store().get_doc_page(repo_id, slug)
    if page is None:
