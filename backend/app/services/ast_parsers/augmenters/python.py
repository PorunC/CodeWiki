...
class PythonSymbolVisitor(py_ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: py_ast.ClassDef) -> None:
        ...
        decorators = [python_expr_name(item) for item in node.decorator_list]
        bases = [python_expr_name(item) for item in node.bases]
        symbol_type = "schema" if is_python_schema_class(node, bases, decorators) else "class"
        self.symbols.append(
            AstSymbol(
                id=symbol_id,
                type=symbol_type,
                ...
                references=python_references(node),
                hash=self.file_hash,
    ...
    def _add_function(self, node: py_ast.FunctionDef | py_ast.AsyncFunctionDef) -> None:
        ...
        self.symbols.append(
            AstSymbol(
                ...
                end_line=getattr(node, "end_lineno", node.lineno),
                parent_id=parent_id,
                signature=signature_line(self.lines, node.lineno),
                docstring=py_ast.get_docstring(node),
                decorators=decorators,
                calls=python_calls(node),
                references=python_references(node),
        ...
        for route in python_endpoint_decorators(node):
            method = route["method"]
...
def python_imports(tree: py_ast.AST) -> list[str]:
    imports: list[str] = []
...
def python_exports(tree: py_ast.AST) -> list[str]:
    for node in py_ast.walk(tree):
...
def python_calls(node: py_ast.AST) -> list[str]:
    calls: set[str] = set()
...
def python_references(node: py_ast.AST) -> list[str]:
    ...
    for child in py_ast.walk(node):
        if isinstance(child, py_ast.Name) and isinstance(child.ctx, py_ast.Load):
            ...
            reference = python_expr_name(child.annotation)
            if reference:
            ...
            reference = python_expr_name(child.returns)
            if reference:
                references.add(reference.rsplit(".", 1)[-1])
...
def python_endpoint_decorators(node: py_ast.FunctionDef | py_ast.AsyncFunctionDef) -> list[dict[str, str]]:
    ...
    for decorator in node.decorator_list:
        ...
        if method not in HTTP_METHODS:
            continue
        route_object = python_expr_name(decorator.func.value)
...
def is_python_schema_class(
    node: py_ast.ClassDef,
    bases: list[str],
    decorators: list[str],
) -> bool:
...
def python_expr_name(node: py_ast.AST) -> str:
    if isinstance(node, py_ast.Name):
        return node.id
    if isinstance(node, py_ast.Attribute):
        base = python_expr_name(node.value)
        return f"{base}.{node.attr}" if base else node.attr
    if isinstance(node, py_ast.Call):
        return python_expr_name(node.func)
    if isinstance(node, py_ast.Subscript):
        return python_expr_name(node.value)
    return ""


def signature_line(lines: list[str], line_number: int) -> str | None:
