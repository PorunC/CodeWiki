...
class ModelRouter:
    ...
    def profile_for(self, task_type: str) -> ModelProfile:
        ...
        profile_config = self._configured_profile(task_type)
        fallback_profile_config = (
            self._configured_profile(defaults.profile_key) if defaults.profile_key else None
        )
        return self._profile(
            task_type,
            profile_config,
            fallback_profile_config=fallback_profile_config,
    ...
    def _profile(
        ...
        *,
        fallback_profile_config: LLMProfileSettings | None = None,
        temperature: float = 0.1,
        max_tokens: int | None = None,
        stream: bool = False,
    ) -> ModelProfile:
        default = self.settings.llm.default
        fallback = fallback_profile_config or LLMProfileSettings()
        return ModelProfile(
            task_type=task_type,
            model=self._value(profile_config.model, fallback.model, default.model) or "provider/strong-coding-model",
            provider_type=self._value(profile_config.provider_type, fallback.provider_type, default.provider_type),
            endpoint=self._value(profile_config.endpoint, fallback.endpoint, default.endpoint),
            api_key=self._value(profile_config.api_key, fallback.api_key, default.api_key),
            temperature=temperature,
            max_tokens=self._max_tokens(profile_config, fallback, default, max_tokens),
            stream=stream,
        )
    ...
    def _max_tokens(
        profile_config: LLMProfileSettings,
        fallback_profile_config: LLMProfileSettings,
        default_profile_config: LLMProfileSettings,
        task_default: int | None,
    ) -> int | None:
        for value in (
            profile_config.max_tokens,
            fallback_profile_config.max_tokens,
            default_profile_config.max_tokens,
            task_default,
        ):
            if value is None:
                continue
