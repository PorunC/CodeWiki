...
| 数据库 | SQLite, FTS5, sqlite-vec metadata |
...
RepoScanResult
  ...
  -> SQLiteStore.replace_graph / replace_graph_communities
```
...
query
  ...
  -> search_fts(code_chunk_fts)
  -> optional search_vectors(sqlite-vec + embedding profile)
...
CLI 和 API 共享同一组 service 和 SQLite store。
...
3. Embedding vector search 是可选路径，默认 GraphRAG 主要依赖 symbol、FTS 和 graph expansion。
